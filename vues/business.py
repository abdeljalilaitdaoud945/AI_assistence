...
def build(page: ft.Page) -> ft.View:
    ...
    def open_edit_dialog(deal=None):
        ...
        f_notes = ft.TextField(
            label="Notes", value=deal.get("notes", ""),
            multiline=True, min_lines=2, max_lines=4,
            bgcolor=C.bg_subtle, border_color=C.border, focused_border_color=C.accent, color=C.text,
            label_style=ft.TextStyle(color=C.text_subtle),
        )

        # ---- Ajout : Génération IA Offre ----
        ...
        def _save(e):
            try:
                montant = float(f_montant.value) if f_montant.value else None
            except Exception:
                montant = None
            ...
            if is_new:
                add_deal(
                    client=f_client.value or "Client", intitule=f_intitule.value or "Sans titre",
                    montant=montant, stage=f_stage.value or "demande",
                    contact_email=f_email.value or "", notes=final_notes,
                )
            else:
                update_deal(
                    deal["id"], client=f_client.value, intitule=f_intitule.value,
                    montant=montant, stage=f_stage.value,
                    contact_email=f_email.value, notes=final_notes,
                )
            page.pop_dialog()
            refresh()
                ...
                controls=[
                    ft.Row(
                        ...
                        controls=[
                            ...
                            ft.Container(
                                padding=ft.Padding(left=8, top=2, right=8, bottom=2), border_radius=999, bgcolor=ft.Colors.with_opacity(0.16, color),
    ...
    def refresh():
        ...
        if not items:
            ...
            for d in items:
                deals_col.controls.append(_deal_card(d))
        page.update()
    ...
    add_btn = ft.IconButton(
        icon=ft.Icons.ADD_ROUNDED, icon_color="#FFFFFF", bgcolor=C.accent_strong, tooltip="Nouveau dossier",
